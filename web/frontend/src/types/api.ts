...
export interface BondLadderRequest {
...
export interface BondLadderMetric {
    ticker: string;
    name: string;
...
export interface BondLadderResponse {
    series: TimeSeries[];
    metrics: BondLadderMetric[];
}
...
export interface BacktestCostAssumptions {
    commission_mode: "none" | "per_share" | "percentage";
...
export interface AppliedBacktestCostAssumptions extends BacktestCostAssumptions {
...
export interface BacktestCostEventRecord {
    timestamp: string;
    ticker: string;
    cost_type:
        | "commission"
...
export interface BacktestCostSummary {
    ...
    total_costs: number;
    costs_by_symbol: Record<string, number>;
    cost_events: BacktestCostEventRecord[];
}

export interface MissingDataTickerSummary {
    ticker: string;
    rows_before: number;
    rows_after: number;
    rows_dropped: number;
    ...
    remaining_missing_cells: number;
    had_missing_data: boolean;
}

export interface BacktestMissingDataSummary {
    policy: MissingDataPolicy;
    total_missing_rows: number;
    total_missing_cells: number;
    remaining_missing_cells: number;
    note?: string | null;
    tickers: MissingDataTickerSummary[];
}
...
export interface BacktestResponse {
    ...
    cost_summary?: BacktestCostSummary | null;
    ...
    walk_forward_request?: WalkForwardHandoff | null;
}
...
export interface MultiAssetMonteCarloRequest {
    tickers: string[];
    weights?: number[];
    sim_periods: number;
    n_sims: number;
...
export interface MultiAssetWeight {
    ticker: string;
    weight: number;
}

export interface MultiAssetAssetStat {
    ticker: string;
    weight: number;
...
export interface MultiAssetMonteCarloResponse {
    periods: number[];
    portfolio_bands: PercentileBand[];
    portfolio_sample_paths: (number | null)[][];
    portfolio_statistics: Record<string, number | null>;
    weights: MultiAssetWeight[];
    asset_statistics: MultiAssetAssetStat[];
...
export interface ParetoOptimizerRequest {
    tickers: string[];
    strategies?: string[];
    start_date?: string;
    end_date?: string;
...
export interface ParetoOptimizerResponse {
    objective_a_name: string;
    objective_b_name: string;
    n_evaluated: number;
    all_points: ParetoPointResponse[];
    pareto_front: ParetoPointResponse[];
    dominated_points: ParetoPointResponse[];
    warnings: string[];
}
...
export interface EfficientFrontierAssetStat {
    ticker: string;
...
export interface EfficientFrontierResponse {
    portfolios: EfficientFrontierPortfolio[];
    frontier: EfficientFrontierPortfolio[];
    max_sharpe: EfficientFrontierPortfolio;
    min_volatility: EfficientFrontierPortfolio;
    asset_stats: EfficientFrontierAssetStat[];
    correlation_matrix: Record<string, Record<string, number>>;
}
...
export interface WalkForwardHandoff extends WalkForwardRequest {
    initial_cash: number;
    train_window: number;
    test_window: number;
    step_size: number;
