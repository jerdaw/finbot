...
export default function WalkForwardPage() {
    return (
        <Suspense fallback={<WalkForwardPageFallback />}>
            <WalkForwardPageContent />
        </Suspense>
    );
}

function WalkForwardPageFallback() {
    return (
...
function WalkForwardPageContent() {
    ...
    useEffect(() => {
        const nextTickers = searchParams.get("tickers");
        ...
        const nextInitialCash = searchParams.get("initial_cash");
        const nextTrainWindow = searchParams.get("train_window");
        const nextTestWindow = searchParams.get("test_window");
        const nextStepSize = searchParams.get("step_size");
        const nextAnchored = searchParams.get("anchored");
        const nextIncludeTrain = searchParams.get("include_train");
        const nextStrategyParams = searchParams.get("strategy_params");
        ...
        if (nextInitialCash) {
            const parsed = Number(nextInitialCash);
            if (Number.isFinite(parsed) && parsed > 0) {
                setInitialCash(parsed);
            }
        }
        if (nextTrainWindow) {
            const parsed = Number(nextTrainWindow);
            if (Number.isFinite(parsed) && parsed >= 21) {
                setTrainWindow(parsed);
            }
        }
        if (nextTestWindow) {
            const parsed = Number(nextTestWindow);
            if (Number.isFinite(parsed) && parsed >= 5) {
                setTestWindow(parsed);
            }
        }
        if (nextStepSize) {
            const parsed = Number(nextStepSize);
            if (Number.isFinite(parsed) && parsed >= 1) {
                setStepSize(parsed);
            }
        }
        if (nextAnchored) {
            setAnchored(nextAnchored === "true");
        }
        if (nextIncludeTrain) {
            setIncludeTrain(nextIncludeTrain === "true");
        }
        if (nextStrategyParams) {
            try {
                const parsed = JSON.parse(nextStrategyParams) as Record<
                    string,
                    unknown
                >;
                setParams(parsed);
            } catch {
                // Ignore malformed query params and keep interactive defaults.
            }
        }
    }, [searchParams]);
    ...
    const handleRun = () => {
        ...
        if (
            ...
            !Array.isArray(strategyParams.equity_proportions)
        ) {
        ...
        if (
            strategy === "Rebalance" &&
            !Array.isArray(strategyParams.rebal_proportions)
        ) {
                    ...
                    <ConfigPanel title="Configuration">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                value={trainWindow}
                                onChange={(e) =>
                                    setTrainWindow(Number(e.target.value))
                                }
                                min={1}
                            />
                            <p className="text-xs text-muted-foreground">
                                Trading days for in-sample training (504 = ~2
                                years)
                            </p>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                value={testWindow}
                                onChange={(e) =>
                                    setTestWindow(Number(e.target.value))
                                }
                                min={1}
                            />
                            <p className="text-xs text-muted-foreground">
                                Trading days for out-of-sample testing (126 = ~6
                                months)
                            </p>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            />
                            <p className="text-xs text-muted-foreground">
                                Trading days between each window start (63 =
                                ~quarter)
                            </p>
                        </div>
                        ...
                        <p className="text-xs text-muted-foreground">
                            Anchored: training window starts from the same date
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <Switch
                                ...
                                onCheckedChange={setIncludeTrain}
                            />
                        </div>
                        <p className="text-xs text-muted-foreground">
                            Include in-sample training window outputs alongside
                            the out-of-sample test windows.
                        </p>
            ...
            >
                                ...
                                <TabsContent value="summary" className="mt-4">
                                    ...
                                    result.summary_table.length > 0 ? (
                                        ...
                                        <DataTable
                                            ...
                                            data={Object.entries(summary).map(
                                                ([k, v]) => ({
                                                    metric: k
                                                        .replace(/_/g, " ")
                                                        .replace(/\b\w/g, (c) =>
                ...
                {!mutation.isPending && !mutation.isError && !result && (
                    <EmptyState
                        ...
                        presets={[
                            {
                                label: "SPY Rolling Windows",
                                onClick: () => {
                                    setTickers("SPY");
                                    setStrategy("NoRebalance");
                                    setTrainWindow(504);
                                    setTestWindow(126);
                                },
                            },
