...
def build_research_strategy(base_strategy: Any) -> Any:
    ...
    class ResearchWrappedStrategy(base_strategy):
        def __init__(
            self,
            ...
            **kwargs: Any,
            ...
            self._recurring_cashflows = [
                ...
                for rule in (recurring_cashflows or [])
            ]
            self._one_time_cashflows = [
                _OneTimeCashflowRule(
                    amount=float(rule["amount"]),
                    date=_parse_date(rule["date"]),
                    label=str(rule.get("label") or "One-time cashflow"),
                )
                for rule in (one_time_cashflows or [])
            ]
    ...
    ResearchWrappedStrategy.__name__ = f"ResearchWrapped{base_strategy.__name__}"
    return ResearchWrappedStrategy
